...
class FileOrganizer:
    ...
    def get_organization_preview(self, target_path: str) -> Dict[str, Any]:
        """
        Get a preview of what organization would look like without applying changes
        
        Args:
            target_path: Path to analyze
            
        Returns:
            Preview of organization changes
        """
        logger.info(f"Generating organization preview for: {target_path}")
        
        try:
            # Create organization plan
            plan = self.analyze_and_plan_organization(target_path)
            
            # Generate preview structure
            preview = {
                'current_structure': self._get_directory_structure(Path(target_path)),
                'proposed_changes': {
                    'folders_to_create': [op for op in plan['operations'] if op['type'] == 'create_folder'],
                    'files_to_move': [op for op in plan['operations'] if op['type'] == 'move'],
                    'files_to_rename': [op for op in plan['operations'] if op['type'] == 'rename'],
                    'duplicates_to_remove': [op for op in plan['operations'] if op['type'] == 'delete_duplicate']
                },
                'statistics': plan['statistics'],
                'benefits': self._calculate_organization_benefits(plan)
            }
            
            return preview
            
        except Exception as e:
            logger.error(f"Failed to generate organization preview: {e}")
            raise
    
    def _get_directory_structure(self, path: Path, max_depth: int = 3) -> Dict[str, Any]:
        """Get current directory structure for preview"""
        structure = {
            'name': path.name,
            'type': 'directory',
            'children': []
        ...
        try:
            items = list(path.iterdir())[:20]  # Limit items for performance
            ...
            for item in items:
                if item.is_file():
                    ...
                    structure['children'].append(
                        self._get_directory_structure(item, max_depth - 1)
                    )
    ...
    def _calculate_organization_benefits(self, plan: Dict[str, Any]) -> List[str]:
        """Calculate potential benefits of organization"""
        benefits = []
        
        stats = plan['statistics']
        
        if stats['files_to_move'] > 0:
            benefits.append(f"Organize {stats['files_to_move']} files into logical categories")
        
        if stats['folders_to_create'] > 0:
            benefits.append(f"Create {stats['folders_to_create']} new folders for better structure")
        
        if stats['duplicates_found'] > 0:
            benefits.append(f"Remove {stats['duplicates_found']} duplicate files to save space")
        
        categories_count = len(plan['categories_used'])
        if categories_count > 0:
            benefits.append(f"Separate files into {categories_count} distinct categories")
        
        if not benefits:
            benefits.append("Files are already well organized")
